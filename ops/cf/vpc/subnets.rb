@subnets = 2.times.map do |i|
  "Subnet#{i}".tap do |subnet|
    ...
    resource "RouteTable#{subnet}", 'AWS::EC2::SubnetRouteTableAssociation', DependsOn: subnet do
...
output :Subnets, Fn::join(',', @subnets.map(&Fn.method(:ref))), export: Fn::sub('${AWS::StackName}-Subnets')
